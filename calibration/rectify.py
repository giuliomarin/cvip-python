...
calibPath = '/Data/0_Dataset/multicam_out/calib_s3_rect.yml'
s = 1
c1 = 0
c2 = 1
for scene in range(1, 11):
    imgsDir = '/Data/0_Dataset/multicam_out/%d' % scene
    imgpath1 = os.path.join(imgsDir, 'img_r10_s3_c1.png')
    imgpath2 = os.path.join(imgsDir, 'img_r10_s3_c2.png')
    ...
    # Rectification
    newsize = tuple(res_l)
    R_l, R_r, P_l, P_r, _, _, _ = cv2.stereoRectify(K_l, D_l, K_r, D_r, newsize, R, T, flags=cv2.CALIB_ZERO_DISPARITY, alpha=1)
